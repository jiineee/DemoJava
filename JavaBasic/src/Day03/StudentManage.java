	...
	public static void main(String[] args) {
		...
		x:while(true) {
			...
			case "1":
				...
				count ++; //카운트 증가
				// 여기에 인덱스를 하면 중간에 삭제를 했을 시에 인덱스가 복잡해짐 
			...
			case "3":
				 ...
				 *	다음정보출력을 누르면 index를 +1시키로 해당위치에 정보를 출력
				 *	조건
				 *	count-1보다 index가 같거나 크다면, 출력하지 않도록 처리. (이전과 다음으로 잡음)
				 */
			...
			case "5":
				...
				if(index >= 0 && index <= count-1) {
					...
					System.out.println(index + "번쨰위치를 수정합니다.");
			...
			case "6":
				...
				/*
				 * 	현재정보를 삭제
				 * 	현재 삭제하려는 index부터 ~~~뒤에 있는 배열요소를 당겨와서 
				 * 	덮어 씌워줍니다.
				 * 
				 * 	사람수를 감소
				 * 	
				...
				if(index >= 0 && index <= count - 1) { //삭제가능 조건
					...
					count--; //사람 수를 하나 줄여서 마치 삭제한 것 처럼 처리
